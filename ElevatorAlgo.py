...
class ElevatorAlgo:
    ...
    def allocate(self):
        for i in self.call_list:
            ...
            min = self.time_to(self.Elevator_list[0], i.src) + len(self.Elevator_list[0].dest)
            sel_elev = 0
            num = 0
    ...
    def update_position(self, call: Calls):
        ...
        for x in self.Elevator_list:
            ...
            elev_speed = x._speed
            if len(x.dest) > 0:
                timeneed = (abs(x.dest[0] - x.currfloor) / elev_speed) + delay_time
            if x.status == 1:
    ...
    def update_up(self, elev: Elevator, total_time_since: int, time_need4call):
        ...
        if timeleft > 0:  # if there's more time left then we need then send the elevator with the time she need
            ...
            elev.currfloor = math.ceil(curr_floor + (time_need4call - delay_time) * elev_speed)
            timeleft = total_time_since - time_need4call  # update time
        else:  # else the elevator need more time so send her with all the time
            check = elev.currfloor = math.ceil(curr_floor + (total_time_since - delay_time) * elev_speed)
            elev.currfloor = math.floor(curr_floor + (total_time_since - delay_time) * elev_speed)
            ...
            if len(elev.dest) == 0:  # if theres no more calls for the elevator then make her status 0
                ...
                if timeleft > 0:  # if theres time left after she got to her closest call then
                    ...
                    if elev.dest[0] > elev.currfloor:  # test formula to calc the floor
                        ...
                        elev.currfloor = math.ceil(curr_floor + (timeleft - delay_time) * elev_speed)
                    else:
                        # if the elevator need to go down
                        elev.currfloor = math.floor(curr_floor - (timeleft - delay_time) * elev_speed)
    ...
    def update_down(self, elev: Elevator, total_time_since: int, time_need4call):
        ...
        if timeleft > 0:  # if there's more time left then we need then send the elevator with the time she need
            check = math.floor(curr_floor - (time_need4call - delay_time) * elev_speed)
            elev.currfloor = math.floor(curr_floor - (time_need4call - delay_time) * elev_speed)
            timeleft = total_time_since - time_need4call
        else:  # else the elevator need more time so send her with all the time
            check = math.floor(curr_floor - (total_time_since - delay_time) * elev_speed)
            elev.currfloor = math.ceil(curr_floor - (total_time_since - delay_time) * elev_speed)
